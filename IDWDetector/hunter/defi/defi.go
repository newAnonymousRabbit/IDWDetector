...
func LoadDefi(filePath string, createDataset bool, createSeed bool) (map[string]ProxyInfo) {
	...
	for{
		...
		// create related dir
		if createDataset{
			err = os.Mkdir("hunter/dataset/" + proxyName, os.ModePerm)
			err = os.Mkdir("hunter/dataset/" + proxyName + "/proxy", os.ModePerm)
			err = os.Mkdir("hunter/dataset/" + proxyName + "/stake", os.ModePerm)
		}
		if createSeed{
